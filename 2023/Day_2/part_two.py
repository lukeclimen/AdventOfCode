...
from part_one import _number_of_a_colour, get_colour_block_selection


def find_game_colour_limit(game: dict) -> int:
    """Returns the power set of the fewest number of cubes of each colour."""
    red_minimum = 0
    green_minimum = 0
    blue_minimum = 0
    for game_subset in game["game_subsets"]:
        for pull in game_subset:
            ...
            if number_colour_dict["colour"] == "red":
                if number_colour_dict["number"] > red_minimum:
                    red_minimum = number_colour_dict["number"]
            elif number_colour_dict["colour"] == "blue":
                if number_colour_dict["number"] > blue_minimum:
                    blue_minimum = number_colour_dict["number"]
            elif number_colour_dict["number"] > green_minimum:
                green_minimum = number_colour_dict["number"]
...
def sum_possible_games(game_list: list) -> int:
    ...
    for game in game_list:
        sum += find_game_colour_limit(game)
    return sum
...
if __name__ == "__main__":
    ...
    print(
        "The sum of power sets from each game totals: ",
        sum_possible_games(list_of_games),
    )
